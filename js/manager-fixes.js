...
(async function(){
  try{
    ...
    const isManager=document.getElementById('tbody');
    if(!isManager) return;
    ...
    const ds=devices||[];
    const kpi=document.getElementById('devices');
    if(kpi) kpi.textContent=String(ds.length);
    ...
    const visitCard=[...document.querySelectorAll('.card')].find(c=>c.querySelector('h2')?.textContent.includes('سجل الزيارات'));
    if(container) container.insertBefore(card,visitCard||container.lastElementChild);
  ...
  }catch(e){console.error('manager-fixes',e)}
})();